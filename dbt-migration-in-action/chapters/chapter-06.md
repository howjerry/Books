...
> 在本章中，我們將處理更複雜的場景：**分區資料表**（Partitioned Tables）。分區是 BigQuery 中最重要的性能優化技術之一，正確遷移分區配置對於保持查詢效率和控制成本至關重要。到本章結束時，你將掌握時間分區和整數分區的遷移方法，並將這些知識整合到 Migration Playbook v2.0 中。
...
### 更新 Migration Playbook

我們將分區表模式加入 Migration Playbook v2.0：
...
- ✅ 整數分區遷移範例
- ✅ 自動化驗證腳本（分區配置、性能、成本）
- ✅ Migration Playbook v2.0（新增分區表章節）
