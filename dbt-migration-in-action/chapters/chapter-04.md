...
> 在本章中，我們將建立一份專門寫給 AI 閱讀的遷移手冊（Migration Playbook）。這是整個專案的轉折點。到本章結束時，你將擁有一個包含 6 個核心章節的遷移手冊 v1.0，並且能夠用它指導 Claude Code 完成第二個 SQL 檔案的遷移，效率和準確性都將大幅提升。
...
這就是遷移手冊（Migration Playbook）誕生的背景。
...
這是一個重要的思維轉換：**遷移手冊不是寫給人類的文檔，而是寫給 AI 的操作手冊**。
...
```markdown
【提示詞】
請參考遷移手冊 v1.0 將 daily_sales.sql 轉換為 dbt 模型。
...
在這個提示詞中：
- **「遷移手冊 v1.0」** 提供了所有的知識和規則
- **「daily_sales.sql」** 指定了具體的任務對象
- **輸出要求** 明確了預期的產出物

🔍 **深入理解**：把遷移手冊想像成「教科書」，提示詞想像成「作業題目」。教科書提供知識和方法，作業題目指定具體要做什麼。

## 4.2 建立第一版手冊

現在讓我們開始建立遷移手冊 v1.0。這個初版將專注於最基本的遷移場景：**每日完全更新資料表**。
...
**下一步行動**：
1. 將遷移手冊 v1.0 保存為獨立文件（如 `playbook-v1.0.md`）
